clear all
addpath('/Users/Runze/Documents/GitHub/Dissertation/Code/MATLAB/BrainVis/NII')
...
boundaryVec = 1:70;
boundaryVec(vertexVec)=[];
for j = 1:length(boundaryVec)
    V = double(maskCopy == boundaryVec(j));
    FV{length(vertexVec)+j+1} = isosurface(V, .5);
    p = patch(FV{length(vertexVec)+j+1});
    isonormals(double(maskCopy~=0),p)
    p.FaceColor = colors(length(vertexVec)+j+1,:);
    p.EdgeColor = 'none';
    p.FaceAlpha = 0.05;
%     lighting gouraud
%     material dull
